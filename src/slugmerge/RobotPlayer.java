...
//			new Direction(PI), new Direction(-PI / 4), new Direction(-PI / 2), new Direction(-3 * PI / 4)};
    ...
    static final float farmerSpacingY = farmerSpacing * 0.86602540f;
...
//			}
	...
	private static void pickFarmLocation() {
		try{
    		...
    		float y_t =  rc.readBroadcastFloat(996);
    		float y_b = rc.readBroadcastFloat(997);
    		float y = Math.round(rc.getLocation().y / (farmerSpacingY)) * farmerSpacingY;
			float yx = Math.round(rc.getLocation().y / (farmerSpacingY)) * farmerSpacing;
	    	float yy = Math.round(rc.getLocation().y / (farmerSpacingY * 2)) * farmerSpacing * 2;
	    	float x = Math.round(rc.getLocation().x / (farmerSpacing)) * farmerSpacing + 0.5f * (yx - yy);
	    	MapLocation location = new MapLocation(x, y);
	    	if (freeSpace(location, 1f) > 0) {
	    		if((x_r != -1f && location.x > x_r - 2) || (x_l != 10000f && location.x < x_l + 2) 
	    				|| (y_t != -1f && location.y > y_t - 2)  || (y_b != 10000f && location.y < y_b + 2)){
	    		} else {
		    ...
		    for (int i = 0; i < 6; i++) {
		    	Direction direction  = farmDirections[(counter + i) % 6];
		    	MapLocation newLocation = location.add(direction, farmerSpacing);
		    	rc.setIndicatorDot(newLocation, 124, 124, 124);
